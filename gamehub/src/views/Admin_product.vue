  ...
  >
    ...
    <template v-slot:[`item.actions`]="{ item }">
      <v-icon
        small
        class="mr-2"
        @click.prevent="updateProduct(newproduct)"
        @click="editItem(item)"
      >
        mdi-pencil
      </v-icon>
      <v-icon small @click="deleteProducts(item.id)">
        mdi-delete
      </v-icon>
    </template>
  </v-data-table>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: "categories_id", value: "categories_id" },
  ...
  methods: {
    ...
    editItem(item) {
      this.editedIndex = this.$store.state.allProducts.indexOf(item);
    ...
    deleteItemConfirm() {
      this.$store.state.allProducts.splice(this.editedIndex, 1);
      this.closeDelete();
    },
    ...
    save() {
      if (this.editedIndex > -1) {
        Object.assign(
          this.$store.state.allProducts[this.editedIndex],
          this.newproduct
        );
      } else {
        this.$store.state.allProducts.push(this.newproduct);
      }
      this.close();
    },
  },
